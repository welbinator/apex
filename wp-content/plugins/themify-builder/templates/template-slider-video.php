...
if (!empty($args['settings']['video_content_slider'])):?>
    <?php foreach ($args['settings']['video_content_slider'] as $video): ?>
        <div class="swiper-slide">
		    ...
		    ?>
                    <div class="tf_rel tf_lazy tf_overflow slide-image"<?php echo !empty($video['video_width_slider'])? 'style="max-width:' . $video['video_width_slider']. 'px;"' : ''; ?>>
	                <?php 
	                if ( $isLocal===true) {
		               echo wp_video_shortcode(array('src' => $video['video_url_slider'],'preload'=>'none'));
	                }else{
	                    echo '<div class="video-wrap" data-url="'.$video['video_url_slider'].'"></div>';
                    }
			?>		
                    </div><!-- /video-wrap -->
                <?php endif; ?>
