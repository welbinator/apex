...
class Tbp_Templates{
	...
	public static function themify_skip_post_options_metabox( $metaboxes, $post_type ) {
	...
	/**
	 * Add meta box for template options
	 *
	 * @since 2.0.8
	 */
	public static function add_meta_boxes() {
		add_meta_box( 'tbp-template', __( 'Template Settings', 'tbp' ), array( __CLASS__, 'metabox_callback' ), self::$post_type );
	}

	/**
	 * Renders Template Settings meta box
	 *
	 * @since 2.0.8
	 */
	public static function metabox_callback( $post ) {
		$active_theme = get_post_meta( $post->ID, 'tbp_associated_theme', true );
		if ( empty( $active_theme ) ) {
			$active_theme = Tbp::get_active_theme()->post_name;
		}
		$themes = Tbp_Themes::prepare_themes_for_js();

		$template_type = get_post_meta( $post->ID, 'tbp_template_type', true );
		if ( empty( $template_type ) && Tbp_Utils::is_multilingual() && isset( $_GET['source_lang'], $_GET['trid'] ) ) {
			$original_post = Tbp_Utils::wpml_get_trid_source_post( $_GET['trid'], $_GET['source_lang'] );
			if ( $original_post ) {
				$template_type = get_post_meta( $original_post, 'tbp_template_type', true );
			}
		}
		?>

		<div class="themify_field_row">
			<div class="themify_field_title"><?php _e( 'Associated Theme', 'tbp' ) ?></div>
			<div class="themify_field">
				<select name="tbp_associated_theme">
				<?php foreach ( $themes as $theme ) : ?>
					<option value="<?php echo esc_attr( $theme['id'] ); ?>" <?php selected( $theme['id'], $active_theme ); ?>><?php echo esc_html( $theme['name'] ); ?></option>
				<?php endforeach; ?>
				</select>
			</div>
		</div>
		...
		<?php if ( $template_type ) : ?>
			<input type="hidden" name="tbp_template_type" value="<?php echo esc_attr( $template_type ); ?>" />
		<?php endif; ?>
	...
	/**
	 * Handle saving options in Template Options meta box
	 *
	 * @since 2.0.8
	 */
	public static function save_post( $post_id ) {
		...
		if ( ! isset( $_POST['tbp_associated_theme'], $_POST['post_type'] ) ) {
			return $post_id;
		}
		...
		$associated_theme = sanitize_text_field( $_POST['tbp_associated_theme'] );
		update_post_meta( $post_id, 'tbp_associated_theme', $associated_theme );

		if ( ! empty( $_POST['tbp_template_type'] ) ) {
			update_post_meta( $post_id, 'tbp_template_type', sanitize_text_field( $_POST['tbp_template_type'] ) );
		}
	}
}
